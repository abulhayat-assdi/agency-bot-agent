...
export function AiControls({ accounts, defaultQuestion, selectedAccountId, selectedPreset, conversationId }: { accounts: MetaAdAccount[]; defaultQuestion: string; selectedAccountId?: string; selectedPreset: DateRangePreset; conversationId?: string }) {
  return (
    <form className="grid gap-4 rounded-3xl border border-border/80 bg-card/80 dark:border-white/10 dark:bg-card p-4 lg:grid-cols-[1fr_180px_180px_auto]" action="/ai-analyst">
      ...
      <label className="space-y-2">
        ...
        <input
          ...
          className="h-11 w-full rounded-2xl border border-input bg-card/70 dark:border-white/10 dark:bg-muted px-3 text-sm outline-none ring-primary/30 transition focus:ring-2"
      ...
      <label className="space-y-2">
        ...
        <select name="accountId" defaultValue={selectedAccountId ?? accounts[0]?.id} className="h-11 w-full rounded-2xl border border-input bg-card/70 dark:border-white/10 dark:bg-muted px-3 text-sm outline-none ring-primary/30 transition focus:ring-2">
      ...
      <label className="space-y-2">
        ...
        <select name="preset" defaultValue={selectedPreset} className="h-11 w-full rounded-2xl border border-input bg-card/70 dark:border-white/10 dark:bg-muted px-3 text-sm outline-none ring-primary/30 transition focus:ring-2">
          {presets.map((preset) => (
